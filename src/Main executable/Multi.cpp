...
void DoCmdGodmode(byte NI)
{
	...
	// 2) включаем Ђбог-режимї и форсируем вместимость
	GodMode = true;
//	NATIONS[NI].NFarms = INT_MAX;
	NATIONS[NI].NFarms = 1000000; // ”станавливаем фиксированное значение 1 миллион
...
// ѕереключение режима перемири€ Ђна летуї без таймера
void Getpeace()
{
	PeaceMode = !PeaceMode;
	...
	if (PeaceMode)
	{
		// ¬ключаем мир Ч ставим бесконечно большой счЄтчик
		PeaceTimeLeft = 0x7FFFFFFF;
	}
	else
	{
		// ¬ыключаем мир Ч обнул€ем счЄтчик
		PeaceTimeLeft = 0;
	}
}
...
{
	ExBuf[EBPos] = 205; // ”никальный код команды
	ExBuf[EBPos + 1] = NI;
	EBPos += 2;
}
//House0 
void DoGetHouse0 ()
{
	ExBuf[EBPos] = 206; // ”никальный код команды
}
void DoGetnocost()
{
	ExBuf[EBPos] = 207; // ”никальный код команды
}
void DoGetpeace()
{
	ExBuf[EBPos] = 208; // ”никальный код команды
}
